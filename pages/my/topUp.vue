...
<script>
	export default {
		...
		onShow() {
			this.getAccount();
			this.$http.get("index/aliNotify").then(res=>{})
		},
		watch: {
		...
		methods: {
			...
			async pay() {
				...
				*/
				try {
					let res = await this.$http.post("goods/wap_adpays", {
						money: this.money
					});
					if(res.code==1){
			...
			async appPay() {
				try {
					let res = await this.$http.post("goods/alipay_adpays", {
						money: this.money
					});
					console.log(res);
					if(res.code==1){
						if (res.data.status == "succeeded") {
							plus.runtime.openURL(res.data.expend.pay_info);
						} else {
							this.toast("支付错误");
						}
					}
