	...
	<view class="content container">
		...
		<view class="tBox">
			...
			<view class="flex plr-30 flex-list">
				...
				<view class="text-center width30percent" @tap="go('/pages/hecheng/hechangDetail')">
			...
			<view class="goodsItem" v-for="(item, index) in collectionList" :key="index" @tap="goDetail(item.id)"
				...
				<view class="flex">
					<view class="labelwrapper">
						...
						<text class="ml-16">{{item.count}}份</text>
					</view>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			goDetail(a) {
				if (this.showType == 1) {
					this.go("art?id=" + a + "&status=" + this.showType);
				} else if (this.showType == 2) {
					this.go("sell?id=" + a + "&status=" + this.showType);
				}
			},
			async getList() {
				let data = {
				...
				try {
					let res = await this.$http.post("index/GoodsUsersList", data);
